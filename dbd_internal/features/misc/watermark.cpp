...
void features::misc::watermark::draw() {
	const auto draw_list = ImGui::GetBackgroundDrawList();

	const auto fps = static_cast<int>(1.f / hooks::delta_time);
	const auto latency = features::esp::players::latency;

	std::string text;
	text.append("dbd_internal | fps: ").append(std::to_string(fps)).append(" | latency: ").append(std::to_string(latency)).append("ms");
