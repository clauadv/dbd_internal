...
bool ImGui::Hotkey(const char* label, int* k, const ImVec2& size_arg) {
    ...
    const ImGuiID id = window->GetID(label);
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const ImRect frame_bb(window->DC.CursorPos + ImVec2(label_size.x + style.ItemInnerSpacing.x, 0.0f), window->DC.CursorPos + size_arg);
    ...
    if (!hovered)
        window->DrawList->AddRectFilled(frame_bb.Min, frame_bb.Max, GetColorU32(ImGuiCol_FrameBg));
    else
        window->DrawList->AddRectFilled(frame_bb.Min, frame_bb.Max, GetColorU32(ImGuiCol_FrameBgHovered));
...
bool ImGui::Checkbox(const char* label, bool* v, bool unsafe)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    if (label_size.x > 0.0f) {
        if (unsafe) {
            RenderText(ImVec2(check_bb.Max.x + style.ItemInnerSpacing.x, check_bb.Min.y + style.FramePadding.y), label, 0, true, ImGuiCol_TextUnsafe);
        } else {
