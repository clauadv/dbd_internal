...
namespace hooks {
	...
	inline void post_render_hk(sdk::UObject* game_viewport_client, sdk::UCanvas* canvas) {
		...
		for (int i = 0; i < players.size(); i++) {
			...
			const auto actor_name = players[i]->PlayerNamePrivate();
			...
			canvas->K2_DrawText(actor_name->c_str(), screen_position, sdk::FVector2D{ 1.0f, 1.0f }, color, 1.0f, sdk::FLinearColor{ 0, 0, 0, 0 }, sdk::FVector2D{ 0, 0 }, true, false, false, sdk::FLinearColor{ 0, 0, 0, 1.f });
		}
