...
namespace ImGui
{
    ...
    IMGUI_API bool          Checkbox(const char* label, bool* v, bool unsafe = false);
...
struct ImFontGlyphRangesBuilder
{
    ...
    inline bool     GetBit(int n) const {
        int off = (n >> 5); ImU32 mask = 1u << (n & 31); return (UsedChars[off] & mask) != 0;
    }  // Get bit n in the array
    inline void     SetBit(int n) {
        int off = (n >> 5); ImU32 mask = 1u << (n & 31); UsedChars[off] |= mask;
...
struct ImFontAtlasCustomRect
{
    ...
    ImFontAtlasCustomRect() {
        ID = 0xFFFFFFFF; Width = Height = 0; X = Y = 0xFFFF; GlyphAdvanceX = 0.0f; GlyphOffset = ImVec2(0, 0); Font = NULL;
    }
    bool IsPacked() const {
        return X != 0xFFFF;
    }
